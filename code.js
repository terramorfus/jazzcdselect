...
function removeFromCart (index) {
    // Tomo el producto y lo borro del array cart
...
function getTotal() {
    let total = 0;
      //  lo que queremos hacer es un loop que tome cada uno de los artículos del carrito y vaya sumando su precio al total
    ...
    for (let i = 0; i < cart.length; i++) { // El for se compone de 4 partes
        ...
        // Obtenemos el precio del item y lo sumammos al total
...
function purchase(index) {
    ...
    document.querySelector('#subtotal-' + index).innerHTML = '£' + (product.price * product.quantity) // + (product.price * product.quantity).toFixed(); 
}
